      ...
      <div className='w-full flex flex-wrap gap-4 gap-y-6'>
        {
          doctors.map((item, index) => (
            <div className='border border-indigo-200 rounded-xl max-w-40 overflow-hidden cursor-pointer group' key={index}>
               <div className="h-[50%] w-full bg-indigo-50 flex items-center justify-center group-hover:bg-primary transition-all duration-500">
    <img 
      className="w-full h-full object-cover" 
      src={
        item.image 
          ? `${import.meta.env.VITE_BACKEND_URL + item.image}` 
          : "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAOEAAADhCAMAAAAJbSJIAAAAV1BMVEX6+vq..."
      } 
      alt="" 
    />
  </div>
